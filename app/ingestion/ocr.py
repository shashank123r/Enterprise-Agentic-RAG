...
class OCRProcessor:
    ...
    async def process_pdf_page(self, pdf_path: str | Path, page_number: int, dpi: int = 300) -> tuple[str, bool]:
        ...
        def _do() -> tuple[str, bool]:
            ...
            with TemporaryDirectory() as tmpdir:
                images = convert_from_path(str(pdf_path), first_page=page_number, last_page=page_number, dpi=dpi, output_folder=tmpdir, fmt="png", thread_count=2)
                if not images:
                    return "", False
                text = pytesseract.image_to_string(images[0], config=f"--oem 3 --psm 6 -l {lang} --dpi 300")
    ...
    async def process_image_bytes(self, image_data: bytes, page_number: int | None = None) -> tuple[str, bool]:
    ...
    async def process_full_pdf(self, pdf_path: str | Path, dpi: int = 200) -> tuple[dict[int, str], bool]:
        ...
        def _do() -> tuple[dict[int, str], bool]:
            ...
            ocr_applied = False
            with TemporaryDirectory() as tmpdir:
                images = convert_from_path(str(pdf_path), dpi=dpi, output_folder=tmpdir, fmt="png", thread_count=4)
