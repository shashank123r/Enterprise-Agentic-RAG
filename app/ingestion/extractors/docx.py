...
class DocxExtractor(BaseExtractor):
    ...
    async def extract(self) -> ExtractionResult:
        def _do_extract() -> ExtractionResult:
            ...
            r.metadata.update({
                ...
                "category": props.category or "", "keywords": props.keywords or "",
            ...
            for para in doc.paragraphs:
                ...
                if para.style and "Heading" in (para.style.name or ""):
                    if page_text:
                        r.pages.append(PageResult(page_number=len(r.pages) + 1, text="\n".join(page_text), section_title=current_section))
                        page_text = []
            ...
            for rel in doc.part.rels.values():
                if rel.reltype == RT.IMAGE:
                    ...
                    r.images.append(ImageResult(page_number=None, image_index=image_count, image_data=rel.target_part.blob, format=ext))
    ...
    async def extract_metadata(self) -> dict:
        def _do_meta() -> dict:
            ...
            doc = DocxDocument(str(self.file_path))
            props = doc.core_properties
            return {"title": props.title or "", "author": props.author or "", "created": str(props.created) if props.created else "", "modified": str(props.modified) if props.modified else "", "last_modified_by": props.last_modified_by or "", "category": props.category or "", "keywords": props.keywords or "", "revision": props.revision or 0, "page_count": len(doc.paragraphs)}
